...
namespace its
{
    ...
    {
        private const string ErrorControllerName = "its.Controllers.ErrorController";
            ...
            string[] args)
        {
            ...
            var instance = config[Keys.Configuration.Instance] ?? "its";
            if (!string.IsNullOrEmpty(instance))
            {
                loggerConfig.Enrich.WithProperty(InstanceEnrichment, instance);
            }

            var rollingLogLocation = config[Keys.Configuration.RollingLogLocation];
            if (!string.IsNullOrEmpty(rollingLogLocation))
            {
                ...
                string httpErrorFileTag = config[Keys.Configuration.HttpErrorFileTag];
